...
func (s *Service) get(ctx context.Context, endpoint string) (io.ReadCloser, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to connect to teku HTTP endpoint")
	}
	...
	if statusFamily != 2 {
		...
		return nil, fmt.Errorf("HTTP call to teku endpoint failed with status %d: %s", resp.StatusCode, string(data))
	}
