...
func TestAggregateAndProofDomain(t *testing.T) {
	tests := []struct {
		name string
	}{
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			beaconAttesterDomain, err := service.AggregateAndProofDomain(context.Background())
			require.NoError(t, err)
			require.NotNil(t, beaconAttesterDomain)
		})
	}
}
