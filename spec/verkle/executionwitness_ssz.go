...
func (s *SuffixStateDiff) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (1) 'CurrentValue'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(s.CurrentValue)

	// Offset (2) 'NewValue'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(s.NewValue)
...
func (s *SuffixStateDiff) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'CurrentValue'
	{
		...
		if cap(s.CurrentValue) == 0 {
			s.CurrentValue = make([]byte, 0, len(buf))
		}
		s.CurrentValue = append(s.CurrentValue, buf...)
	}
	...
	// Field (2) 'NewValue'
	{
		...
		if cap(s.NewValue) == 0 {
			s.NewValue = make([]byte, 0, len(buf))
		}
		s.NewValue = append(s.NewValue, buf...)
	}
	return err
}
...
func (s *StemStateDiff) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	for ii := 0; ii < len(s.SuffixDiffs); ii++ {
		offset += 4
		offset += s.SuffixDiffs[ii].SizeSSZ()
	}
...
func (s *StemStateDiff) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'SuffixDiffs'
	{
		...
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if err = s.SuffixDiffs[indx].UnmarshalSSZ(buf); err != nil {
				return err
			}
			return nil
		})
