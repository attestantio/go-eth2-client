...
func TestBeaconBlockHeaderJSON(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:  "JSONBad",
			input: []byte("[]"),
			err:   "invalid JSON: json: cannot unmarshal array into Go value of type altair.beaconBlockHeaderJSON",
		},
		{
			name:  "SlotMissing",
...
func TestBeaconBlockHeaderSpec(t *testing.T) {
	...
	baseDir := filepath.Join(os.Getenv("ETH2_SPEC_TESTS_DIR"), "tests", "mainnet", "altair", "ssz_static", "BeaconBlockHeader", "ssz_random")
