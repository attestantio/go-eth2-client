...
func TestSignedVoluntaryExitJSON(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:  "JSONBad",
			input: []byte("[]"),
			err:   "invalid JSON: json: cannot unmarshal array into Go value of type altair.signedVoluntaryExitJSON",
		},
		{
			name:  "MessageMissing",
		...
		},
		{
			...
			err:   "invalid JSON: invalid JSON: json: cannot unmarshal bool into Go value of type altair.voluntaryExitJSON",
		},
		{
			name:  "MessageInvalid",
...
func TestSignedVoluntaryExitSpec(t *testing.T) {
	...
	baseDir := filepath.Join(os.Getenv("ETH2_SPEC_TESTS_DIR"), "tests", "mainnet", "altair", "ssz_static", "SignedVoluntaryExit", "ssz_random")
