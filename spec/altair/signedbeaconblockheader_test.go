...
func TestSignedBeaconBlockHeaderJSON(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:  "JSONBad",
			input: []byte("[]"),
			err:   "invalid JSON: json: cannot unmarshal array into Go value of type altair.signedBeaconBlockHeaderJSON",
		},
		{
			name:  "MessageMissing",
		...
		},
		{
			...
			err:   "invalid JSON: invalid JSON: json: cannot unmarshal bool into Go value of type altair.beaconBlockHeaderJSON",
		},
		{
			name:  "MessageInvalid",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			var res spec.SignedBeaconBlockHeader
...
func TestSignedBeaconBlockHeaderSpec(t *testing.T) {
	...
	baseDir := filepath.Join(os.Getenv("ETH2_SPEC_TESTS_DIR"), "tests", "mainnet", "altair", "ssz_static", "SignedBeaconBlockHeader", "ssz_random")
