...
func TestSignedAggregateAndProofJSON(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:  "JSONBad",
			input: []byte("[]"),
			err:   "invalid JSON: json: cannot unmarshal array into Go value of type altair.signedAggregateAndProofJSON",
		},
		{
			name:  "MessageMissing",
		...
		},
		{
			...
			err:   "invalid JSON: invalid JSON: json: cannot unmarshal bool into Go value of type altair.aggregateAndProofJSON",
		},
		{
			name:  "MessageInvalid",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			var res spec.SignedAggregateAndProof
...
func TestSignedAggregateAndProofSpec(t *testing.T) {
	...
	baseDir := filepath.Join(os.Getenv("ETH2_SPEC_TESTS_DIR"), "tests", "mainnet", "altair", "ssz_static", "SignedAggregateAndProof", "ssz_random")
