...
func (s *BeaconState) MarshalJSON() ([]byte, error) {
	...
	previousEpochParticipation, err := MarshalParticipationFlags(s.PreviousEpochParticipation)
	if err != nil {
		return nil, errors.Wrap(err, "previous_epoch_participation")
	}

	currentEpochParticipation, err := MarshalParticipationFlags(s.CurrentEpochParticipation)
	if err != nil {
...
func (s *BeaconState) unpack(data *beaconStateJSON) error {
	...
	if s.PreviousEpochParticipation, err = ParseParticipationFlags(data.PreviousEpochParticipation, "previous_epoch_participation"); err != nil {
		return err
	}

	if s.CurrentEpochParticipation, err = ParseParticipationFlags(data.CurrentEpochParticipation, "current_epoch_participation"); err != nil {
		return err
	}
