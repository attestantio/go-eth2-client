...
func TestProposerSlashingJSON(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:  "BadJSON",
			input: []byte(`[]`),
			err:   "invalid JSON: json: cannot unmarshal array into Go value of type altair.proposerSlashingJSON",
		},
		{
		...
		},
		{
			...
			err:   "invalid JSON: invalid JSON: json: cannot unmarshal bool into Go value of type altair.signedBeaconBlockHeaderJSON",
		},
		{
...
func TestProposerSlashingSpec(t *testing.T) {
	...
	baseDir := filepath.Join(os.Getenv("ETH2_SPEC_TESTS_DIR"), "tests", "mainnet", "altair", "ssz_static", "ProposerSlashing", "ssz_random")
