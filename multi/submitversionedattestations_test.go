...
func TestSubmitVersionedAttestations(t *testing.T) {
	...
	for i := 0; i < 128; i++ {
		err := multiClient.(consensusclient.VersionedAttestationsSubmitter).SubmitVersionedAttestations(ctx, &api.SubmitAttestationsOpts{})
		require.NoError(t, err)
	}
