...
func (s *Service) BeaconAttesterDomain(ctx context.Context) ([]byte, error) {
	if s.beaconAttesterDomain == nil {
		...
		val, exists := cfg["domain_beacon_attester"]
		if !exists {
			return nil, errors.New("failed to obtain domain_beacon_attester value")
		}
		floatVal, isNum := val.(float64)
		if !isNum {
			return nil, errors.New("domain_beacon_attester value not a number")
		}
		intVal := uint32(floatVal)
		var domain [4]byte
		binary.LittleEndian.PutUint32(domain[:], intVal)
		s.beaconAttesterDomain = domain[:]
	}
