...
func (s *Service) SelectionProofDomain(ctx context.Context) ([]byte, error) {
	if s.selectionProofDomain == nil {
		...
		val, exists := cfg["domain_selection_proof"]
		if !exists {
			return nil, errors.New("failed to obtain domain_selection_proof value")
		}
		floatVal, isNum := val.(float64)
		if !isNum {
			return nil, errors.New("domain_selection_proof value not a number")
		}
		intVal := uint32(floatVal)
		var domain [4]byte
		binary.LittleEndian.PutUint32(domain[:], intVal)
		s.selectionProofDomain = domain[:]
	}
