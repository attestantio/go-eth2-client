...
func (s *Service) RANDAODomain(ctx context.Context) ([]byte, error) {
	if s.randaoDomain == nil {
		...
		val, exists := cfg["domain_randao"]
		if !exists {
			return nil, errors.New("failed to obtain domain_randao value")
		}
		floatVal, isNum := val.(float64)
		if !isNum {
			return nil, errors.New("domain_randao value not a number")
		}
		intVal := uint32(floatVal)
		var domain [4]byte
		binary.LittleEndian.PutUint32(domain[:], intVal)
		s.randaoDomain = domain[:]
	}
