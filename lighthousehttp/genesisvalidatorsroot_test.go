...
func TestGenesisValidatorsRoot(t *testing.T) {
	tests := []struct {
		name string
	}{
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			genesisValidatorsRoot, err := service.GenesisValidatorsRoot(context.Background())
			require.NoError(t, err)
			require.NotNil(t, genesisValidatorsRoot)
		})
	}
}
