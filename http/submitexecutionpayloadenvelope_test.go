...
func TestSubmitExecutionPayloadEnvelope(t *testing.T) {
	...
	t.Run("PostsEnvelope", func(t *testing.T) {
		received := false
		server := httptest.NewServer(nethttp.HandlerFunc(func(w nethttp.ResponseWriter, r *nethttp.Request) {
			...
			case "/eth/v1/beacon/execution_payload_envelopes":
				...
				require.Equal(t, "true", r.Header.Get("Eth-Blob-Data-Included"))
		...
		require.NoError(t, err)
		err = localService.(client.ExecutionPayloadEnvelopeSubmitter).SubmitExecutionPayloadEnvelope(ctx, &api.SubmitExecutionPayloadEnvelopeOpts{
			SignedExecutionPayloadEnvelope: &spec.VersionedSignedExecutionPayloadEnvelope{
