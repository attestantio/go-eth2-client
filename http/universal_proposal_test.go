...
func TestUniversalProposal(t *testing.T) {
	...
	tests := []struct {
		...
		expected *api.VersionedUniversalProposal
		err      string
		errCode  int
	}{
		{
		...
		{
			name: "Good with builder boost factor",
			opts: &api.UniversalProposalOpts{
				RandaoReveal: phase0.BLSSignature([96]byte{
				...
				Slot:                   phase0.Slot(uint64(time.Since(genesisResponse.Data.GenesisTime).Seconds())/uint64(slotDuration.Seconds())) + 1,
				SkipRandaoVerification: true,
				BuilderBoostFactor:     "1",
			},
		},
		{
			name: "Good without builder boost factor",
			opts: &api.UniversalProposalOpts{
				RandaoReveal: phase0.BLSSignature([96]byte{
					0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
					0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			response, err := service.(client.UniversalProposalProvider).UniversalProposal(ctx, test.opts)
			switch {
			case test.err != "":
